...
public class CommonSpliter extends Spliter {
	...
	private static HashSet<String> separatorTable = new HashSet();

	static {
		for (int i = 0; i < SEPARATOR.length; ++i) {
			separatorTable.add(SEPARATOR[i]);
		}
		char[] chars = "@_abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789０１２３４５６７８９ａｂｃｄｅｆｇｈｉｊｋｌｍｎｏｐｑｒｓｔｕｖｗｘｙｚＡＢＣＤＥＦＧＨＩＪＫＬＭＮＯＰＱＲＳＴＵＶＷＸＹＺ"
				.toCharArray();
	...
	public static String[] split(String s, int apw, boolean chineseEnglishSplited) {
		List list = new ArrayList();
		if (chineseEnglishSplited)
			list.addAll(Arrays.asList(getChineseEnglishSeparatedString(s)));
		else {
			list.add(s);
		}
		...
		for (int i = 0; i < list.size(); ++i) {
			...
			if (i > 0) {
				if (!(StringUtil.isAllChineseCharacter((String) list.get(i - 1))))
					last = (String) list.get(i - 1);
				else {
					last = null;
				}
			}
			if (!(StringUtil.isAllChineseCharacter(ss))) {
				result.add(ss);
			} else {
				String[] vs = split(ss, apw);
				result.addAll(Arrays.asList(vs));
				if ((vs.length > 0) && (last != null)) {
					result.add(last + vs[0]);
				}
			}
		}
	...
	public static String[] extractCommonWord(String sentence) {
		...
		char[] chars = sentence.toCharArray();
		for (int i = 0; i < chars.length; ++i) {
			if ("@_abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789０１２３４５６７８９ａｂｃｄｅｆｇｈｉｊｋｌｍｎｏｐｑｒｓｔｕｖｗｘｙｚＡＢＣＤＥＦＧＨＩＪＫＬＭＮＯＰＱＲＳＴＵＶＷＸＹＺ"
