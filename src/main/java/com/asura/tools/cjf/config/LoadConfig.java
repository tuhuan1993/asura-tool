package com.asura.tools.cjf.config;
...
public class LoadConfig {
	...
	public Char[] loadFJmapUTF8() throws IOException {
		...
		try {
			...
			while ((line = br.readLine()) != null) {
				...
				if (index >= fChar) {
					list = null;
					throw new IOException("对不起，您的配置文件[fj_map_utf8.properties]有问题！FCharId=" + fChar);
				}
				index = fChar;
	...
	protected BufferedReader getReader(String file) {
		...
		try {
			InputStream is = super.getClass().getResourceAsStream(file);
